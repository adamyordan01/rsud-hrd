...
class PegawaiAktifExport extends DefaultValueBinder implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnFormatting, WithCustomValueBinder
{
    private $bulan;
    private $tahun;
    ...
    public function bindValue(Cell $cell, $value)
    {
        ...
        $textColumns = ['A', 'B', 'C', 'D', 'K', 'Y', 'Z', 'AA', 'AB', 'AC', 'AD', 'AE', 'AH', 'AI', 'AK', 'AL', 'AM', 'AN', 'BQ', 'BS'];
    ...
    /**
     * Query data pegawai aktif - menggunakan raw SQL seperti sistem HRD original
     */
    public function collection()
    {
        // Query yang sama dengan sistem HRD original tapi disesuaikan untuk Laravel
        if ($this->bulan == date('m') && $this->tahun == date('Y')) {
            // Data current period - menggunakan VIEW_TAMPIL_KARYAWAN dengan STR dan SIP
            $sql = "SELECT DISTINCT k.*, 
                    ...
                    OUTER APPLY (
                        SELECT TOP 1 NO_STR, TGL_KADALUARSA 
                        FROM HRD_R_STR 
                        WHERE KD_KARYAWAN = k.KD_KARYAWAN 
                        ORDER BY TGL_KADALUARSA DESC
                    ) str 
                    OUTER APPLY (
                        SELECT TOP 1 NO_SIP, TGL_KADALUARSA 
                        FROM HRD_R_SIP 
                        WHERE KD_KARYAWAN = k.KD_KARYAWAN 
                        ORDER BY TGL_KADALUARSA DESC
                    ) sip
                    WHERE STATUS_PEG = 1 
                    ORDER BY k.KD_KARYAWAN";
        } else {
            // Data backup period
            $sql = "SELECT * 
                    FROM VIEW_TAMPIL_KARYAWAN_BACKUP 
                    WHERE STATUS_PEG = 1 
                        AND BULAN_BACKUP = ? 
                        AND TAHUN_BACKUP = ? 
        ...
        if ($this->bulan == date('m') && $this->tahun == date('Y')) {
            ...
            return collect(DB::connection('sqlsrv')->select($sql, [$this->bulan, $this->tahun]));
        }
    }
     ...
     * Field names disesuaikan dengan lowercase karena PDO::ATTR_CASE => PDO::CASE_LOWER
     */
    public function map($row): array
    {
        // Format tanggal helper - menggunakan format dd/mm/yyyy
        ...
        // Format nomor - HILANGKAN single quote, biarkan CustomValueBinder yang handle
        $formatNumber = function($number) {
            if (empty($number)) return '';
            return (string) $number; // Cast sebagai string saja, tanpa quote
        };

        // Jenis kelamin - biarkan Wanita/Pria (tidak dikonversi ke P/L)
        ...
        return [
            ...
            $formatNumber($row->no_askes ?? ''),                  // NO ASKES / BPJS KESEHATAN
            $formatNumber($row->no_bpjs_ketenagakerjaan ?? ''),   // BPJS KETENAGAKERJAAN
            ...
            $formatNumber($row->rek_bni_syariah ?? ''),           // NO REKENING BNI SYARIAH
    ...
    public function columnFormats(): array
    {
        return [
            ...
            'D' => NumberFormat::FORMAT_TEXT,  // NIP BARU ← PENTING untuk 197312202007011005
            'K' => NumberFormat::FORMAT_TEXT,  // NO KTP ← PENTING untuk 1103043112700022
            ...
            'AB' => NumberFormat::FORMAT_TEXT, // NO ASKES / BPJS KESEHATAN
            'AC' => NumberFormat::FORMAT_TEXT, // BPJS KETENAGAKERJAAN
            'AD' => NumberFormat::FORMAT_TEXT, // NO TASPEN
            'AE' => NumberFormat::FORMAT_TEXT, // NO NPWP
            'AF' => NumberFormat::FORMAT_TEXT, // NO KK
            'AI' => NumberFormat::FORMAT_TEXT, // NO HP
            'AJ' => NumberFormat::FORMAT_TEXT, // NO HP ALTERNATIF
            'AL' => NumberFormat::FORMAT_TEXT, // NO REKENING BPD ACEH
            'AM' => NumberFormat::FORMAT_TEXT, // NO REKENING BNI
            'AN' => NumberFormat::FORMAT_TEXT, // NO REKENING BNI SYARIAH
            'AO' => NumberFormat::FORMAT_TEXT, // NO REKENING MANDIRI
            'BQ' => NumberFormat::FORMAT_TEXT, // NO STR
            'BS' => NumberFormat::FORMAT_TEXT, // NO SIP
        ];
    }
