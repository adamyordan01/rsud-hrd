...
class PegawaiPerawatBidanExport extends DefaultValueBinder implements FromCollection, WithHeadings, WithMapping, WithStyles, WithColumnFormatting, WithCustomValueBinder, WithEvents
{
    private $bulan;
    private $tahun;
    ...
    /**
     * Query data perawat-bidan sesuai dengan sistem HRD original
     * Menggabungkan data PERAWAT dan BIDAN dengan STR/SIP untuk RS Online
     * Dengan pemisah antara PERAWAT dan BIDAN
     */
    public function collection()
    {
        // Query data PERAWAT (KD_DETAIL_JENIS_TENAGA = 1)
        $sqlPerawat = "SELECT k.*, 
                           'PERAWAT' as KATEGORI_DETAIL,
                           'DATA' as ROW_TYPE,
                           str.NO_STR, 
                       ...
                       WHERE k.KD_JENIS_TENAGA = 2 
                         AND k.KD_DETAIL_JENIS_TENAGA = 1
                         AND k.STATUS_PEG = 1
                       ORDER BY k.SUB_DETAIL ASC, 
                                k.KD_STATUS_KERJA ASC, 
                                k.NAMA ASC";

        // Query data BIDAN (KD_DETAIL_JENIS_TENAGA = 2)
        $sqlBidan = "SELECT k.*, 
                         'BIDAN' as KATEGORI_DETAIL,
                         'DATA' as ROW_TYPE,
                         str.NO_STR, 
                         str.TGL_KADALUARSA as STR_TGL_KADALUARSA, 
                         sip.NO_SIP, 
                     ...
                     WHERE k.KD_JENIS_TENAGA = 2 
                       AND k.KD_DETAIL_JENIS_TENAGA = 2
                       AND k.STATUS_PEG = 1
                     ORDER BY k.SUB_DETAIL ASC, 
                              k.KD_STATUS_KERJA ASC, 
                              k.NAMA ASC";

        $dataPerawat = collect(DB::connection('sqlsrv')->select($sqlPerawat));
        $dataBidan = collect(DB::connection('sqlsrv')->select($sqlBidan));

        // Buat collection gabungan dengan pemisah
        $result = collect();

        // Tambah header PERAWAT
        $headerPerawat = (object) [
            'row_type' => 'HEADER',
            'kategori_detail' => 'PERAWAT',
            'nama' => 'PERAWAT'
        ];
        $result->push($headerPerawat);

        // Tambah data PERAWAT
        $result = $result->merge($dataPerawat);

        // Tambah header BIDAN
        $headerBidan = (object) [
            'row_type' => 'HEADER',
            'kategori_detail' => 'BIDAN',
            'nama' => 'BIDAN'
        ];
        $result->push($headerBidan);

        // Tambah data BIDAN
        $result = $result->merge($dataBidan);
     ...
     * 27 kolom termasuk STR/SIP untuk perawat-bidan (tidak ada NO ABSEN)
     */
     ...
     * Dengan handling untuk row pemisah
     */
    public function map($row): array
    {
        // Jika ini adalah row header pemisah
        if (isset($row->row_type) && $row->row_type === 'HEADER') {
            $result = array_fill(0, 27, ''); // Buat array 27 kolom kosong
            $result[0] = $row->nama; // Isi kolom pertama dengan nama kategori (PERAWAT/BIDAN)
            return $result;
        }
        ...
        return [
            ...
            $formatNumber($row->no_sip ?? ''),                    // NO SIPP - dari subquery
    ...
    public function columnFormats(): array
    {
        return [
            ...
            'X' => NumberFormat::FORMAT_TEXT,  // NO SIPP
    ...
    /**
     * Register events untuk styling row pemisah
     */
        ...
        return [
            AfterSheet::class => function(AfterSheet $event) {
                ...
                foreach ($data as $row) {
                    // Jika ini adalah row header pemisah
                    if (isset($row->row_type) && $row->row_type === 'HEADER') {
                        // Merge kolom A sampai AA (27 kolom) untuk row pemisah
                        $sheet->mergeCells("A{$currentRow}:AA{$currentRow}");
                        
                        // Style untuk row pemisah sesuai original (background biru)
