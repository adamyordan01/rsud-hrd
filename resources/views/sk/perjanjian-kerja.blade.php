...
<head>
    ...
    <style>
        @font-face {
            ...
            font-style: normal;
            font-weight: normal;
            src: url('/fonts/Bookman_Old_Style_Regular.ttf') format('truetype');
        }
        ...
        <table width="90%" style="margin:0 auto;margin-top:20px;">
            <tr>
                <td colspan="5" class="text-justify">
                    ...
                    <b
                        style='text-transform:capitalize;'>
                        {{-- ".penyebut(date_format($result['TGL_SK'], 'Y'))." --}}
                        {{-- {{ penyebut(date_format($results->tgl_sk, 'Y')) }} --}}
                        {{ penyebut(date('Y', strtotime($results->tgl_sk))) }}
