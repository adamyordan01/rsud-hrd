            ...
            <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
                        ...
                        <li class="breadcrumb-item text-muted">Bahasa</li>
                    </ul>
                </div>
                <div class="d-flex align-items-center gap-2 gap-lg-3">
                    <a href="javascript:;" class="btn btn-flex btn-primary h-40px fs-7 fw-bold" id="add-bahasa">
                        Tambah Bahasa
                    </a>
                </div>
            </div>
            ...
            <div class="card-header border-0 pt-6">
                <div class="card-title">
                    <div class="d-flex align-items-center position-relative my-1">
                        ...
                        <input type="text" data-kt-user-table-filter="search" class="form-control form-control-solid w-250px ps-12" placeholder="Cari Bahasa ...">
                    </div>
                </div>
            </div>
            <div class="card-body pt-0">
                <div id="kt_customers_table_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                    <div class="table-responsive">
                        <table id="bahasa-table" class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer text-gray-600 fw-semibold sortable">
                            <thead>
                                <tr>
                                    <th class="w-10px">No.</th>
                                    <th class="">Nama Bahasa</th>
        ...
        <div class="modal-dialog mw-550px">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="fw-bold modal-title" id="modalTitle">Tambah Bahasa</h2>
                ...
                <form method="POST" class="form" id="bahasaForm">
                    ...
                    <input type="hidden" name="kd_bahasa" id="kd_bahasa_hidden">
                    <div class="modal-body">
                        <!-- Nama Bahasa -->
                        <div class="d-flex flex-column mb-3">
                            <label class="fw-semibold fs-6 mb-2 required">Bahasa</label>
                            <input type="text" class="form-control form-control-solid" name="bahasa" id="bahasa" placeholder="Bahasa" autofocus />
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            var table = $('#bahasa-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: '{{ route("admin.settings.bahasa.index") }}',
                    type: 'GET'
                },
                columns: [
                    { 
                ...
                createdRow: function(row, data, dataIndex) {
                    $(row).attr('data-id', data.kd_bahasa);
                },
            ...
            $('#add-bahasa').on('click', function() {
                $('#modalTitle').text('Tambah Bahasa');
                $('#formMethod').val('POST');
                $('#status_aktif').val('1');
                $('#bahasaForm')[0].reset();
                $('.error-text').text('');
            ...
            $('#bahasa-table').on('click', '.edit', function() {
                var id = $(this).data('id');
                $.ajax({
                    url: '{{ route("admin.settings.bahasa.edit", ":id") }}'.replace(':id', id),
                    type: 'GET',
                    success: function(response) {
                        $('#modalTitle').text('Edit Bahasa');
                        $('#formMethod').val('PATCH');
                        $('#kd_bahasa_hidden').val(response.data.kd_bahasa);
                        $('#bahasa').val(response.data.bahasa);
                        $('.error-text').text('');
                        $('#bahasaModal').modal('show');
                    },
                    error: function(xhr) {
            ...
            $('#bahasaForm').on('submit', function(e) {
                ...
                var url = method === 'POST' 
                    ? '{{ route("admin.settings.bahasa.store") }}' 
                    : '{{ route("admin.settings.bahasa.update", ":id") }}'.replace(':id', $('#kd_bahasa_hidden').val());
            ...
            $('#bahasaModal').on('hidden.bs.modal', function() {
                $('#bahasaForm')[0].reset();
                $('.error-text').text('');
            });
        });
