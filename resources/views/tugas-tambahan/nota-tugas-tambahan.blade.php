...
<head>
    ...
    <title>Nota Tugas Tambahan</title>
    <style>
        @page {
            footer: page-footer;
        }
...
<body>
    @php
        ...
        //     $jenistenaga = "";
        // }
        $jenis_tenaga = $getVerifikasi->sub_detail ?? "";
        $ruangan = $getVerifikasi->ruangan ?? "";
        ...
        if ($kd_status_kerja == 1) {
            $status_kerja = "NIP. " . $getVerifikasi->nip_baru . " - " . "ID Peg : " . $getVerifikasi->kd_karyawan . "<br>" . $getVerifikasi->pangkat . " (" . ($getVerifikasi->alias_gol ?? '') . ")<br>" . $getVerifikasi->jab_fung;
        } else if ($kd_status_kerja == 7) {
            // NIPPPK. 199801022024050001 - ID Peg : 001635
            // Golongan VIII
            // Teknisi Elektromedis Penyelia
            // buat seperti bentuk di atas
            $status_kerja = "NIPPPK. " . $getVerifikasi->nip_baru . " - " . "ID Peg : " . $getVerifikasi->kd_karyawan . "<br>" . "Golongan " . $getVerifikasi->alias_gol . "<br>" . $getVerifikasi->jab_fung;
        ...
        // $isi_nota_ruangan = {"Selanjutnya anda mendapatkan tugas tambahan sebagai $getVerifikasi->sub_detail_tambahan di ruangan $getVerifikasi->ruangan_tambahan"};
        $isi_nota_ruangan = "Selanjutnya anda mendapatkan tugas tambahan sebagai " . $getVerifikasi->sub_detail_tambahan . " di ruangan " . $getVerifikasi->ruangan_tambahan;
    @endphp
    ...
    <div class="all">
        ...
        <table class="table-print t-font" width="85%">
            ...
            <tr>
                <td colspan=2>TMT AWAL</td>
                <td>:</td>
                <td colspan='2'>
                    {{ tanggal_indo($getVerifikasi->tmt_awal) }}
                </td>
            </tr>
            <tr>
                <td colspan=2>TMT AKHIR</td>
                <td>:</td>
                <td colspan='2'>
                    {{ $getVerifikasi->tmt_akhir ? tanggal_indo($getVerifikasi->tmt_akhir) : '-' }}
                </td>
            </tr>
            <tr>
            ...
            {{-- @if ($getVerifikasi->ruangan_tambahan && $getVerifikasi->sub_detail_tambahan)
                <tr>
            ...
            <tr>
                ...
                <td width="380px" style="text-align: center; line-height: 20px;">
                    ...
                    <img
                        src="{{ storage_path('app/public/qr-code-nota-tugas-tambahan/' . $year . '/' . $getVerifikasi->kd_tugas_tambahan . '-' . $getVerifikasi->kd_karyawan . '.png') }}"
                        alt="QR Code"
                        style="width: 130px; height: 130px; margin-top: 10px; margin-bottom: 10px;"
                    /><br>
                    <b><u>{{ $nama_lengkap_direktur }}</u></b><br>
                    {{ $getDirektur->pangkat }} ({{ $getDirektur->alias_gol }})<br>
