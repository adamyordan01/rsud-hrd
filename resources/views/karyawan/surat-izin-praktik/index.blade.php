...
@push('scripts')
    <script>
        ...
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.data.sc_berkas) {
                            ...
                            var downloadUrl = "{{ route('admin.karyawan.sip.download', ['id' => ':id', 'urut' => ':urut']) }}";
                            downloadUrl = downloadUrl.replace(':id', response.data.kd_karyawan);
                            downloadUrl = downloadUrl.replace(':urut', response.data.urut_sip);
            ...
            // Handle delete SIP
            $('#sip-table').on('click', '.delete-sip', function(e) {
                e.preventDefault();
                
                var kd_karyawan = $(this).data('karyawan');
                var urut = $(this).data('urut');
                var noSip = $(this).data('no-sip');
                
                // Konfirmasi dengan SweetAlert2
                Swal.fire({
                    title: 'Apakah Anda yakin?',
                    text: 'SIP dengan nomor "' + noSip + '" akan dihapus permanen!',
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        var url = "{{ route('admin.karyawan.sip.destroy', ['id' => ':kd_karyawan', 'urut' => ':urut']) }}";
                        url = url.replace(':kd_karyawan', kd_karyawan);
                        url = url.replace(':urut', urut);
                        ...
                        $.ajax({
                            ...
                            error: function(xhr) {
                                Swal.fire({
                                    title: 'Error!',
                                    text: xhr.responseJSON?.message || 'Terjadi kesalahan saat menghapus SIP.',
                                    icon: 'error'
                                });
                            }
                        });
