            ...
            <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
                <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
                    ...
                    <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
                        ...
                        <li class="breadcrumb-item text-muted">
                            <a href="{{ route('admin.jenis-tenaga.index', $jenisTenaga) }}" class="text-muted text-hover-primary">
                                {{ $titleBreadcrumb }}
                            </a>
                        </li>
                        <li class="breadcrumb-item">
                        ...
                        <li class="breadcrumb-item text-muted">
                            {{ $detailJenisTenaga->detail_jenis_tenaga ?? 'Detail' }}
                        </li>
                    </ul>
                </div>
                <div class="d-flex align-items-center gap-2 gap-lg-3">
                    ...
                    <button class="btn btn-flex btn-secondary h-40px fs-7 fw-bold" onclick="printingDetail()">
...
@section('content')
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <div id="kt_app_content_container" class="app-container container-fluid">
            ...
            {{-- Detail Info Card --}}
            @if($detailJenisTenaga)
            <div class="card mb-5">
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        <div class="symbol symbol-60px me-5">
                            <span class="symbol-label bg-light-primary">
                                <i class="ki-duotone ki-user fs-2hx text-primary">
                        ...
                        <div class="flex-grow-1">
                            <h3 class="text-gray-900 fw-bold mb-1">{{ $detailJenisTenaga->detail_jenis_tenaga }}</h3>
                            <span class="text-muted fw-semibold">{{ $titleBreadcrumb }}</span>
                        </div>
                        <div class="text-end">
                            <a href="{{ route('admin.jenis-tenaga.index', $jenisTenaga) }}" class="btn btn-sm btn-light">
                                <i class="ki-duotone ki-arrow-left fs-2"></i>
                                Kembali ke {{ $titleBreadcrumb }}
                            </a>
                        </div>
                    </div>
                </div>
            ...
            <div class="card">
                <div class="card-header border-0 pt-6">
                    <div class="card-title">
                        <div class="d-flex align-items-center position-relative my-1">
                            ...
                            <input type="text" data-kt-detail-table-filter="search" 
                ...
                <div class="card-body pt-0">
                        ...
                        <div class="row">
                            <div class="col-md-9">
                                ...
                                <div class="d-flex flex-wrap">
                                    ...
                                    <button type="button" class="btn btn-sm btn-primary me-4" data-kt-detail-table-filter="apply">
                                        Apply Filter
                                    </button>

                                    <button type="button" class="btn btn-sm btn-secondary" data-kt-detail-table-filter="reset">
                                        Reset
                                    </button>
                                </div>
                            </div>
                            ...
                            <div class="col-md-3">
                                @if(!empty($allDetailJenisTenaga) && $allDetailJenisTenaga->count() > 0)
                                ...
                                <div class="dropdown w-100">
                                    ...
                                    <ul class="dropdown-menu w-100">
                                        @foreach($allDetailJenisTenaga as $detail)
                                        <li>
                                            <a class="dropdown-item {{ $detail->kd_detail == $kdDetail ? 'active' : '' }}" 
                                               href="{{ route('admin.jenis-tenaga.detail', [$detail->kd_detail, $jenisTenaga]) }}">
                    ...
                    <div class="table-responsive">
                        <table id="karyawan-detail-table" class="table align-middle table-row-dashed fs-6 gy-5 dataTable no-footer text-gray-600 fw-semibold">
                            <thead>
                                <tr>
    ...
    <input type="hidden" id="kd-detail" value="{{ $kdDetail }}">
@endsection
...
@push('scripts')
    <script>
        ...
        // Function untuk melakukan print laporan pegawai detail
        function printingDetail() {
            var jenis = $("#jenis-tenaga").val();
            var detail = $("#kd-detail").val();
            var data1 = document.getElementById("pns").checked ? "1" : ""; // PNS
            var data2 = document.getElementById("honor").checked ? "2" : ""; // HONOR
            var data3 = document.getElementById("kontrak").checked ? "3" : ""; // KONTRAK
            var data4 = document.getElementById("partime").checked ? "4" : ""; // PT
            var data7 = document.getElementById("pppk").checked ? "7" : ""; // PPPK

            // Jika tidak ada yang dicentang, set data1 = "a"
            ...
            if (data1) params.append('data1', data1);
            if (data2) params.append('data2', data2);
            if (data3) params.append('data3', data3);
            if (data4) params.append('data4', data4);
            if (data7) params.append('data7', data7);
        ...
        $(document).ready(function() {
            var table = $('#karyawan-detail-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.jenis-tenaga.detail', [$kdDetail, $jenisTenaga]) }}",
                    type: 'GET',
                    data: function (d) {
            ...
            // Page length change
            $('[data-kt-detail-table-filter="length"]').on('change', function() {
                table.page.len(this.value).draw();
            });

            // Apply filter
            $('[data-kt-detail-table-filter="apply"]').on('click', function() {
                table.draw();
            });

            // Reset filter
            $('[data-kt-detail-table-filter="reset"]').on('click', function() {
